<div class="relative">
    <div x-data="markdown()" x-ref="markdownX" data-key="{{ $key }}" id="markdown-{{ $key }}" class="relative w-full" x-init="init()">
        ...
        <div class="@if(isset($style['toolbar'])){{ $style['toolbar'] }}@else{{ 'relative flex items-center justify-between w-full h-12 overflow-x-hidden rounded bg-gray-50 sm:h-10' }}@endif">
        ...
        @if($section == 'preview')
        <div wire:target="updateContentPreview" class="@if(isset($style['preview'])){{ $style['preview'] }}@else{{ 'h-full bg-white min-h-screen relative z-30 px-5 mt-5 prose md:prose-xl lg:prose-2xl' }}@endif" x-cloak>
            {!! $contentPreview !!}
        </div>
        @endif
        ...
        <div x-show="section == 'help'" class="@if(isset($style['help'])){{ $style['help'] }}@else{{ 'h-full bg-white min-h-screen px-5 pt-10 relative z-30 prose bg-white dark:bg-dark-900 md:prose-xl lg:prose-lg' }}@endif" x-cloak>
    ...
    <script>
        ...
        window.markdown = function() {
            return {
                ...
                initializeAceEditor () {
                    this.codeEditor = ace.edit(this.$refs.ace, {
                        maxLines: 50,
                        minLines: 10,
                        fontSize: 18
                    });
                ...
                setModalHTML (suggestion) {
                    ...
                    modalSuggestionHTML["code"] = `<label for="text" class="block mb-1.5 text-sm font-medium leading-5 text-gray-700 sm:mt-px">
                        ...
                        <div x-ref="ace" class="block w-full mb-2 transition duration-150 ease-in-out form-input sm:text-sm sm:leading-5"></div>`;
                ...
                showSuggestionDropdown () {
                    ...
                    //this.dropdownEl.addEventListener("click", this.clickItem);
                },
                hideSuggestionDropdown () {
                    ...
                    if(this.dropdownEl){
                        //let that = this;
                        //this.dropdownEl.removeEventListener("click", this.clickItem);
